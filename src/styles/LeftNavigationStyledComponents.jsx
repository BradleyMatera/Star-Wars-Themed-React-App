...
export const NavWrapper = styled.nav`
  ...
  left: ${(props) =>
    props.$isMenuOpen ? "0" : "-240px"}; // Sliding menu in and out
  width: 240px; // Width of the navigation
  height: calc(100vh - 60px); // Full height minus header height
  background-color: ${(props) =>
    props.$backgroundColor || "#000"}; // Dynamic background color
...
export const NavLink = styled(Link)`
  ...
  color: ${(props) =>
    props.$linkColor || "#ffd700"}; // Dynamic color for the links
  text-decoration: none; // Removing underline
  font-size: 16px; // Font size for the links
  &:hover {
    color: ${(props) =>
      props.$hoverColor || "#ff4500"}; // Hover color for the links
  }
`;
...
export const HamburgerButton = styled.button`
  ...
  background: rgb(121, 41, 41); // No background
  border: none; // No border
  font-size: 24px; // Font size for the icon
  cursor: pointer; // Pointer cursor on hover
  color: ${(props) =>
    props.$buttonColor || "#ffd700"}; // Dynamic color for the button
`;
